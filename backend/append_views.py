...
from .models import CompletionCertificate, PurchaseOrder
...
class CompletionCertificateViewSet(viewsets.ModelViewSet):
    ...
    def get_valid_vendors(self, request):
        # Get vendors that have at least one FULLY APPROVED PO
        from .models import PurchaseOrder, Vendor
        from apps.purchase.serializers import VendorSerializer
        
        # In a real app we might check approval_status='APPROVED' and maybe other states
        pos = PurchaseOrder.objects.filter(is_active=True).values_list('vendor_id', flat=True)
        # assuming the PO is active and valid. If there is an approval state we filter by it.
        # e.g., PurchaseOrder.objects.filter(approval_status='Approved')
        vendors = Vendor.objects.filter(id__in=pos)
    ...
    def get_valid_pos(self, request):
        ...
        # Return fully approved POs for the selected vendor
        pos = PurchaseOrder.objects.filter(vendor_id=vendor_id, is_active=True)
        # Assuming we filter by some approval status here if available, e.g., approval_status='APPROVED'
...
# I need to insert content_urls before the end of urlpatterns in urls.py.
# But it's easier to just read the file and insert it before the closing bracket of urlpatterns
