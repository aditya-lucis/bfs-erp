<template>
  <div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-gray-100 pb-4">
      <div>
        <h2 class="text-lg font-bold text-gray-800">Master Bank</h2>
        <p class="text-xs text-gray-500 mt-0.5">Manage and configure all banks in the system.</p>
      </div>
        ...
        <table class="w-full text-left border-collapse whitespace-nowrap">
          <thead>
            <tr class="bg-gray-50/80 border-b border-gray-100">
              ...
              <th class="py-3.5 px-4 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Bank Code</th>
          ...
          <tbody class="divide-y divide-gray-50">
            ...
            >
              ...
              <td class="py-3 px-4 text-xs font-mono font-semibold text-bfs-navy">{{ item.bank_code }}</td>
              <td class="py-3 px-4 text-sm font-semibold text-gray-800">{{ item.bank_name }}</td>
              <td class="py-3 px-4 text-sm text-gray-600">{{ item.bank_name_id }}</td>
            ...
            <tr v-if="filteredData.length === 0 && !loading">
              <td colspan="7" class="py-12 text-center">
                <div class="flex flex-col items-center justify-center">
                  ...
                  <p class="text-sm font-medium text-gray-600">No banks found</p>
        ...
        <div class="flex items-center gap-1.5">
          ...
          <div class="flex items-center gap-1 px-2">
            <button 
              ...
              :class="currentTab === page ? 'bg-bfs-navy text-white shadow-md' : 'text-gray-500 hover:bg-gray-200'"
            >
              {{ page }}
            </button>
          </div>
        ...
        <div v-if="modal.show" class="fixed inset-0 z-50 overflow-y-auto">
          ...
          <div class="flex min-h-full items-center justify-center p-4">
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg z-10 overflow-hidden" @click.stop>
              <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-bfs-navy via-bfs-gold to-bfs-navy" />
              ...
              <div class="flex items-center justify-between px-6 py-5 border-b border-gray-100">
                <div class="flex items-center gap-3">
                  ...
                  <h3 class="text-base font-bold text-gray-800">
                    {{ modal.mode === 'add' ? 'Create New Bank' : 'Edit Bank' }}
                  </h3>
                </div>
              ...
              <form @submit.prevent="saveBank">
                <div class="px-6 py-6 space-y-5">
                  <div v-if="modal.mode === 'edit'" class="bg-gray-50 rounded-xl p-3 border border-gray-100 flex items-center justify-between">
                    <span class="text-xs text-gray-500 font-medium">Bank Code</span>
                    <span class="text-sm font-mono font-bold text-bfs-navy">{{ form.bank_code }}</span>
                  </div>

                  <div class="grid grid-cols-2 gap-4">
                    <FormField label="Name (EN)" required>
                      <input v-model="form.bank_name" type="text" class="form-input transition-colors focus:ring-bfs-navy" placeholder="e.g. Bank Central Asia" required />
                    </FormField>
                    <FormField label="Name (ID)" required>
                      <input v-model="form.bank_name_id" type="text" class="form-input transition-colors focus:ring-bfs-navy" placeholder="e.g. Bank Central Asia" required />
                    </FormField>
                  </div>
                  ...
                  <div class="flex items-center p-3 mt-2 bg-gray-50 rounded-xl border border-gray-100 cursor-pointer hover:bg-gray-100 transition-colors" @click="form.is_not_active = !form.is_not_active">
                    <div class="flex-1">
                      ...
                      <p class="text-[10px] text-gray-500 mt-0.5">Toggle to deactivate this bank.</p>
                    </div>
                    <div class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors duration-300" :class="form.is_not_active ? 'bg-red-5 00' : 'bg-gray-300'">
...
const filteredData = computed(() => {
  ...
  return allData.value.filter(item =>
    ...
    item.bank_name_id.toLowerCase().includes(q) ||
...
const paginatedData = computed(() => filteredData.value.slice(paginationStart.value, paginationEnd.value))

watch(searchQuery, () => { currentPage.value = 1 })

const modal = reactive({ show: false, mode: 'add', selectedId: null })
const form = reactive({
  bank_code: '',
  bank_name: '',
  bank_name_id: '',
...
const fetchBanks = async () => {
  loading.value = true
  try {
    const response = await api.get('master-type/master-bank/')
...
const openAddModal = () => {
  ...
  form.bank_name = ''
  form.bank_name_id = ''
  form.order_no = (allData.value.length > 0 ? Math.max(...allData.value.map(b => b.order_no)) : 0) + 1
...
const openEditModal = (item) => {
  ...
  form.bank_name_id = item.bank_name_id
...
const saveBank = async () => {
  ...
  try {
    const payload = {
      ...
      bank_name_id: form.bank_name_id,
      order_no: form.order_no,
      is_not_active: form.is_not_active,
    }
    if (modal.mode === 'add') {
      await api.post('master-type/master-bank/', payload)
      Swal.fire({ icon: 'success', title: 'Created!', text: 'Bank has been created.', timer: 1500, showConfirmButton: false })
    } else {
      await api.patch(`master-type/master-bank/${modal.selectedId}/`, payload)
      Swal.fire({ icon: 'success', title: 'Updated!', text: 'Bank has been updated.', timer: 1500, showConfirmButton: false })
    }
    closeModal()
    fetchBanks()
  } catch (error) {
...
const handleDelete = async (id) => {
  ...
  if (result.isConfirmed) {
    try {
      await api.delete(`master-type/master-bank/${id}/`)
      Swal.fire({
        icon: 'success',
        title: 'Deleted!',
