...
const saveTransactionType = async () => {
  ...
  try {
    ...
    if (modal.mode === 'add') {
      ...
      Swal.fire({ icon: 'success', title: 'Created!', text: 'Transaction Type has been created.', timer: 1500, showConfirmButton: false })
    } else {
      await api.patch(`master-type/transaction-type/${modal.selectedId}/`, payload)
      Swal.fire({ icon: 'success', title: 'Updated!', text: 'Transaction Type has been updated.', timer: 1500, showConfirmButton: false })
    }
    ...
    Swal.fire({
      icon: 'error',
      title: 'Save Failed',
      text: 'Failed to save data. Please check the form and try again.',
      confirmButtonColor: '#1e3a8a'
    })
  } finally {
    saving.value = false
...
const handleDelete = async (id) => {
  const result = await Swal.fire({
    title: 'Are you sure?',
    text: 'You want to delete this Transaction Type?',
    icon: 'warning',
