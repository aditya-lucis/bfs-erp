<template>
  <Panel title="Payment Request Inbox" subtitle="Finance | Payment Request Inbox">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div class="lg:col-span-4 border-r border-gray-100 pr-0 lg:pr-6 flex flex-col gap-4">
        ...
        <div class="relative">
          ...
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Cari nomor Payment Request..."
        ...
        <div v-else-if="filteredRequests.length === 0" class="text-center py-12 border border-dashed border-gray-200 rounded-xl bg-gray-50/50">
          ...
          <p class="text-xs text-gray-400">Tidak ada Payment Request yang membutuhkan persetujuan Anda saat ini.</p>
        </div>
        ...
        <div class="pt-4 border-t border-gray-100">
          <label class="flex items-center gap-2 text-xs text-gray-600 cursor-pointer">
            ...
            />
            Tampilkan semua riwayat pengajuan Payment Request
          </label>
        </div>
      </div>
      ...
      <div class="lg:col-span-8 min-h-[400px] flex flex-col">
        ...
        <div v-if="!selectedRequest" class="flex-1 flex flex-col items-center justify-center text-gray-400 py-20 bg-gray-50/30 rounded-2xl border border-dashed border-gray-100">
          ...
          <p class="text-sm font-medium">Pilih dokumen Payment Request dari panel kiri untuk memproses persetujuan.</p>
        </div>
        ...
        <div v-else class="space-y-6">
          ...
          <div class="bg-gray-50/50 border border-gray-100 rounded-2xl p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p class="text-xs text-gray-500">
                Persetujuan untuk dokumen Payment Request (CBR)
              </p>
            </div>

            <div class="text-left md:text-right border-l md:border-l-0 md:border-r border-gray-200 pl-4 md:pl-0 md:pr-4">
              <span class="text-xs text-gray-400 uppercase tracking-wider block font-medium">Requested Amount</span>
              <button 
          ...
          <!-- Document Signatures Library Block -->
          <div class="border border-gray-100 rounded-2xl p-5 bg-gray-50/30">
            <h5 class="text-sm font-semibold text-gray-700 mb-4 flex items-center gap-2">
              ...
              Tanda Tangan Dokumen (Signature Blocks)
            </h5>
        ...
        <div v-if="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl p-6 z-10 max-h-[90vh] flex flex-col">
            <div class="flex items-center justify-between mb-5">
              <div>
                <h3 class="text-lg font-semibold text-gray-800">Detail Payment Request</h3>
            ...
            <div class="overflow-y-auto flex-1 border border-gray-100 rounded-xl bg-gray-50/50 p-4">
              ...
              <div v-else-if="documentDetail">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 bg-white p-4 rounded-xl border border-gray-100">
                  <div class="col-span-2 md:col-span-4"><span class="text-xs text-gray-500 block">Description</span><span class="font-medium text-sm">{{ documentDetail.description || '-' }}</span></div>
                  
                  <div><span class="text-xs text-gray-500 block">Date</span><span class="font-medium text-sm">{{ documentDetail.date || '-' }}</span></div>
                  <div><span class="text-xs text-gray-500 block">Usage For</span><span class="font-medium text-sm">{{ documentDetail.usage_for || '-' }}</span></div>
                  <div><span class="text-xs text-gray-500 block">Transaction Type</span><span class="font-medium text-sm">{{ documentDetail.transaction_type_display || '-' }}</span></div>
                  <div><span class="text-xs text-gray-500 block">Purchase Invoice</span><span class="font-medium text-sm">{{ documentDetail.purchase_invoice_display || '-' }}</span></div>
                  
                  <div><span class="text-xs text-gray-500 block">Vendor / Payment To</span><span class="font-medium text-sm">{{ documentDetail.payment_to_display || '-' }}</span></div>
                  <div><span class="text-xs text-gray-500 block">Project</span><span class="font-medium text-sm">{{ documentDetail.project_display || '-' }}</span></div>
                  <div><span class="text-xs text-gray-500 block">Status</span><span class="font-medium text-sm">{{ documentDetail.document_status }}</span></div>
                  <div><span class="text-xs text-gray-500 block">Total Amount</span><span class="font-medium text-bfs-gold">{{ formatCurrency(documentDetail.amount) }}</span></div>
                </div>
                  ...
                  <table class="w-full text-left border-collapse text-sm">
                    <thead>
                      <tr class="bg-gray-50 border-b border-gray-200">
                        ...
                        <th class="px-4 py-3 font-semibold text-gray-600 text-right">Quantity</th>
                    ...
                    <tbody>
                      <tr v-for="d in documentDetail.details" :key="d.id" class="border-b border-gray-100 last:border-none hover:bg-gray-50/50">
                        <td class="px-4 py-3">
                          <div class="font-medium text-gray-800">{{ d.item_name || 'N/A' }}</div>
                          <div class="text-xs text-gray-400 mt-0.5">{{ d.item_code || '' }}</div>
                        </td>
                        <td class="px-4 py-3 text-right text-gray-600">{{ d.quantity }}</td>
                        <td class="px-4 py-3 text-right text-gray-600">{{ formatCurrency(d.unit_price) }}</td>
                        <td class="px-4 py-3 text-right text-gray-600 font-medium">{{ formatCurrency(d.total_amount) }}</td>
                      </tr>
...
async function loadInbox() {
  ...
  try {
    const filters = { document_code__startswith: 'CBR_' }
...
async function openDetailModal() {
  ...
  try {
    const res = await api.get(`/accounting/cashbook-request/${selectedRequest.value.document_id}/`)
